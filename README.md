...
正本の作業場所は、このREADMEがあるリポジトリです。`work/` はローカル運用情報を置く領域であり、公開資料や通常のソースコードとは分けて扱います。古いコピーを見つけても、内容と履歴を比較せずに削除・統合しないでください。

正本は、このREADMEと`pom.xml`があるリポジトリ直下です。`target/` は再生成できるビルド出力、`work/` はGit管理外のローカル運用領域です。どちらも正本ソースではありませんが、用途を確認せずに削除しないでください。PC固有の復元物一覧はGit公開対象外のローカル台帳で管理します。
